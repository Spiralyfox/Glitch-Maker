"""Playback engine — always-on stream, zero latency. Optimized: stream survives data swaps."""
import numpy as np
...
class PlaybackEngine:
    ...
    def load(self, audio_data: np.ndarray, sr: int):
        """Load audio — only recreates stream if sr/channels changed."""
        ...
        # Only recreate stream if sample rate or channels changed
        ...
        if sr != self._stream_sr or ch != self._stream_ch or self._stream is None:
            self._ensure_stream()
        # Otherwise stream stays alive, just using new data pointer
