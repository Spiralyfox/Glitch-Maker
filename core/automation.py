...
def apply_automation_multi(audio: np.ndarray, start: int, end: int,
    ...
    auto_params: list of dicts, each with:
      ...
      For "constant": value
      Optional: step, pmin, pmax for quantization/clamping
    """
    _log.info("apply_automation_multi: start=%d end=%d fn=%s params=%d",
              start, end, getattr(process_fn, '__name__', '?'), len(auto_params))
    result = audio.copy()
    start = int(start)
    end = int(end)
    ...
    # State for stateful effects (e.g. filters)
    plugin_state = {}
    chunks_ok = 0
    chunks_err = 0
    ...
    while pos < end:
        ...
        chunk_params = {}
        chunk_params["plugin_state"] = plugin_state
        ...
        for ap in auto_params:
            ...
            # Quantize to step if provided
            if step is not None and step > 0:
                val = round(val / step) * step
                # Cast to int if step is integer-valued
                if step == int(step):
                    val = int(round(val))
        ...
        try:
            ...
            if processed is not None and len(processed) == seg_len:
                ...
                chunks_err += 1
                if chunks_err <= 2:
                    _log.warning("Chunk %d: len mismatch %s vs %d",
                                 pos, len(processed) if processed is not None else None, seg_len)
        except Exception as ex:
            chunks_err += 1
            if chunks_err <= 2:
                _log.warning("Chunk %d error: %s", pos, ex, exc_info=True)
        pos = c_end

    _log.info("Automation done: %d ok, %d failed", chunks_ok, chunks_err)
    return result
...
def apply_automation(audio, start, end, process_fn, base_params,
                     ...
                     curve_points, sr, chunk_size=128):
